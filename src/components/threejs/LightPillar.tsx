...
const LightPillar: React.FC<LightPillarProps> = ({
      ...
      void main() {
        ...
        for(int i = 0; i < ${settings.iterations}; i++) {
          ...
          for(int j = 0; j < ${settings.waveIterations}; j++) {
            float wAngle = 0.4;
            float wx = deformed.x * cos(wAngle) - deformed.z * sin(wAngle);
            float wz = deformed.x * sin(wAngle) + deformed.z * cos(wAngle);
            deformed.x = wx;
            deformed.z = wz;
  ...
  if (!webGLSupported) {
    return (
      <div className={`w-full h-full absolute top-0 left-0 flex items-center justify-center bg-black/10 ${className}`}>
        <span className="text-q-text-tertiary text-sm">WebGL not supported</span>
      </div>
    );
  }
